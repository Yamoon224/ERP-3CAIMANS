<x-app-layout>
    ...
    <div class="az-content-body pd-lg-l-40 d-flex flex-column">
        <div class="az-content-breadcrumb">
            ...
            <span>@lang('locale.category', ['prefix'=>'', 'suffix'=>(app()->getLocale() == 'en' ? 'ie' : 'e') .'s'])</span>
        </div>

        <div class="">
            <div class="float-left">
                <div class="az-content-label mg-b-5">@lang('locale.category', ['prefix'=>'', 'suffix'=>(app()->getLocale() == 'en' ? 'ie' : 'e') .'s'])</div>
                <p class="mg-b-20 text-{{ !empty(session('message')) ? 'success' : 'dark' }}">{{ session('message') ?? __('locale.category_listable') }}</p>
            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: inline-block">
                                ...
                                @php($title = 'category') @php($route = 'categories')
                                <x-updatename :item="$item" :title="$title" :route="$route"></x-updatename>
                            </div>
                            <form action="{{ route('categories.destroy', $item->id) }}" style="display: inline-block" method="post">
                                @csrf
                                @method('DELETE')
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">@lang('locale.category', ['prefix'=>'', 'suffix'=>(app()->getLocale() == 'en' ? 'y' : '')])</h6>
